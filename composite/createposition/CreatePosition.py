...
def create_user_position(username):
    ...
    res_updatestonk = requests.post(
        'http://localhost:5000/account/update/stonks/' + username, json=req)
    if res_createpos and purchasetype == "sell":
        ...
        res_updatepos = requests.post(
            'http://localhost:5011/position/updateSold/' + username, json=content)
...
def create_noti(username):
    req = requests.get('http://localhost:5000/account/' + username)
