...
import requests
errorURL = "http://127.0.0.1:5002/error"
...
def create_error():
    ...
    errorFrom = "sendingNoti"
    email = "testing@email.com"
    userid = "testing123"
    
       
    errorJson = ({        "errorFrom": errorFrom,
...
def send_error(error):
    ...
    message = json.dumps(error, default=str) # convert a JSON object to a string

    # send the message
    # always inform Monitoring for logging no matter if successful or not
    channel.basic_publish(exchange=exchangename, routing_key="handle.error", body=message)
...
if __name__ == "__main__":
    print("This is " + os.path.basename(__file__) + ": creating an error...")
    error = create_error()
    send_error(error)
#    print(get_all())
